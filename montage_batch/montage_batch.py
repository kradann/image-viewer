...
class FolderListWidget(QtWidgets.QListWidget):
	...
	def mousePressEvent(self, event):
		...
		if item:
			self.highlight_current_folder(item)

	def highlight_current_folder(self, item):
		for i in range(self.count()):
			self.item(i).setBackground(QtGui.QColor("#303436"))

		item.setBackground(QtGui.QColor(0,120,215,180))
		self.current_item = item

	def highlight_by_folder_path(self, folder_path: str):
		target = os.path.basename(folder_path.rstrip(os.sep))
		for i in range(self.count()):
			it = self.item(i)
			name = it.text().split()[0]
			if name == target:
				self.highlight_current_folder(it)
...
class ImageMontageApp(QtWidgets.QWidget):
	...
	def prev_folder(self):
		...
		if self.main_folder:
			...
			self.show_batch()
			self.folder_list.highlight_by_folder_path(self.folder_path)
